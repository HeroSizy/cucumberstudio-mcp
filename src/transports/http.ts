...
export class StreamableHttpTransport {
  ...
  constructor(createMcpServer: () => Server, private options: HttpTransportOptions) {
    ...
    return new Promise((resolve, reject) => {
      try {
        this.httpServer = this.app.listen(
          ...
          () => {
            console.error(`🌐 Streamable HTTP transport listening on ${this.options.host || '127.0.0.1'}:${this.options.port}`);

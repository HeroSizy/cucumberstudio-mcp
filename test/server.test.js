...
describe('CucumberStudioMcpServer', () => {
    let mockServer;
    let mockApiClient;
    let mockProjectTools;
    let mockScenarioTools;
    let mockActionWordTools;
    let mockTestRunTools;
    ...
    describe('initialize', () => {
        ...
        it('should throw error when API connection fails', async () => {
            ...
            const server = new CucumberStudioMcpServer();
            await expect(server.initialize()).rejects.toThrow('Failed to connect to Cucumber Studio API. Please check your credentials.');
        });
    });
    describe('runWithTransport', () => {
        ...
        it('should throw error for unsupported transport in instance mode', async () => {
            const server = new CucumberStudioMcpServer();
            await server.initialize();
            await expect(server.runWithTransport('http')).rejects.toThrow('Transport http not supported in instance mode');
        });
    });
    describe('getServer', () => {
